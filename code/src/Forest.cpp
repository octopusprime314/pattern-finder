...
void Forest::MemoryQuery()
{
    ...
    while (!processingFinished)
    {
        ...
        if(overMemoryCountCounter == 1000) {
            ...
            loggingIt.str("");
            loggingIt << "Memory limit now doubled to: " << config.memoryBandwidthMB << std::endl;
            cout << loggingIt.str();
            Logger::WriteLog(loggingIt.str());
        }
                ...
                if (patternsThatCantBeDumped.size() == 0)
                {
                    ...
                    while (iterator != finalMetaDataMap.end())
                    {
                        ...
                        if (iterator->second->size() >= config.minOccurrence)
                        {
                            ...
                            countMutex->lock();
                            stats.SetTotalOccurrenceFrequency(currLevel, static_cast<PListType>(iterator->second->size()));
