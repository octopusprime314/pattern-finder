		...
		for(unsigned int threadIteration = 0; threadIteration <= config.testIterations; threadIteration = config.numThreads)
		{
			...
			for(PListType z = 0; z < fileIterations; z++)
			{
                ...
                if (!prediction)
                {
                    ...
                    std::vector<std::vector<PListType>*>* tempList = new std::vector<std::vector<PListType>*>();
                    for (PListType i = 0; i < 256; i++)
                    {
                        bool dataExists = false;
                        for (PListType j = 0; j < config.numThreads; j++)
                        {
                            if ((*prevPListArray)[(i * config.numThreads) + j] != nullptr && (*prevPListArray)[(i * config.numThreads) + j]->size() > 0)
                            {
                                if (!dataExists) {
                                    dataExists = true;
                                    tempList->push_back(new std::vector<PListType>());
                                }
                                tempList->back()->insert(tempList->back()->end(), (*prevPListArray)[(i * config.numThreads) + j]->begin(), (*prevPListArray)[(i * config.numThreads) + j]->end());
                            }
                        }
                    }
                    ...
                    {
                        if ((*tempList)[i] != nullptr && (*tempList)[i]->size() > 1)
                        {
                            prevPListArray->push_back((*tempList)[i]);
                        }
                    }
