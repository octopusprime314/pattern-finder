...
Forest::Forest(int argc, char **argv)
{
	...
	for(f = 0; f < config.files.size(); f++)
	{
		...
		if (!config.files[f]->copyBuffer->is_open()) 
		{
			...
			config.files[f]->dataInput.clear();
//			config.files[f]->dataInput.reserve(0);
		...
		if(config.files[f]->fileString.size() == 0 && config.usingPureRAM)
		{
			...
			if (config.ing)
			{
				...
				config.files[f]->dataInput.clear();
//				config.files[f]->dataInput.resize(config.files[f]->fileStringSize);
				char * preStr = new char[(config.files[f]->fileStringSize) + 1];
				config.files[f]->copyBuffer->read(preStr, config.files[f]->fileString.size());
				boost::algorithm::split(strInput, preStr, boost::algorithm::is_any_of(","));
				delete[] preStr;
				for (int it=0; it <= strInput.size(); it++)
				{
					cout << "strInput:" << strInput[it] << endl;
					config.files[f]->dataInput[it] = boost::lexical_cast<int>(strInput[it]);
					printf("Integer value:%d\n", config.files[f]->dataInput[it]);
				}
			}
		}

		printf("threadIteration:%d; threadNumber:%d", config.testIterations, config.numThreads);
...
//			memoryQueryThread = new thread(&Forest::MemoryQuery, this);
			...
			printf("prediction bool value:%d\n", prediction); // false RAM; true HD;
			firstLevelProcessedHD = prediction;
			for(unsigned int i = 0; i < config.numThreads; i++)
			{
				cout << "Enter SetCurrentLevel" << endl;
			...
			//				cout << "map file string:" << config.files[f]->fileString << endl;
							...
							cout << "map_key:"<< config.files[f]->fileString.substr((*(*prevPListArray)[i])[0] - (levelInfo.currLevel), levelInfo.currLevel) << endl;
							printf("map_value:%d\n",countMap[config.files[f]->fileString.substr((*(*prevPListArray)[i])[0] - (levelInfo.currLevel), levelInfo.currLevel)]);
			//				cout << "after map file string:" << config.files[f]->fileString << endl;
							...
							if((*prevPListArray)[i]->size() >= 1)
							{
								...
								cout << "string value:"<<config.files[f]->fileString.substr((*(*prevPListArray)[i])[0] - (levelInfo.currLevel), levelInfo.currLevel) << endl;
							}
						}
					}
					...
					cout << "enter SetLevelRecording" << endl;
...
bool Forest::PredictHardDiskOrRAMProcessing(LevelPackage levelInfo, PListType sizeOfPrevPatternCount, PListType sizeOfString)
{
	...
	printf("sizeOfPrevPatternCount:%lu\n", sizeOfPrevPatternCount);
	PListType potentialPatterns = sizeOfPrevPatternCount*256;
	printf("potentialPatterns:%lu\n", potentialPatterns);
...
void Forest::WaitForThreads(vector<unsigned int> localWorkingThreads, vector<future<void>> *localThreadPool, bool recursive, unsigned int level)
{
	...
	//Poll each processing thread to see if it is done processing and then release thread power to the other threads when a thread is released from processing
...
vector<vector<PListType>> Forest::ProcessThreadsWorkLoadRAM(unsigned int threadsToDispatch, vector<vector<PListType>*>* patterns)
{
	...
	for(PListType i = 0; i < threadsToDispatch; i++)
	{
		...
		printf("Threads to be dispatched:%d\n", threadsToDispatch);
	}
	...
	if(patterns->size() == threadsToDispatch*256)
	{
		...
		for(PListType i = 0; i < patternTotals.size(); i++)
		{
				...
				if((*patterns)[(i*threadsToDispatch) + j] != NULL)
				{
					for(PListType z = 0; z < threadsToDispatch; z++)
					{
						if(balancedSizeList[z] < smallestAmount)
						{
							smallestAmount = balancedSizeList[z];
							smallestIndex = z;
							printf("smallestAmount:%d\n smallestIndex:%d\n", smallestAmount, smallestIndex);
							found = true;
						}
					}
			...
			if(found)
			{
				...
				printf("balancedSizeList:%d\n", balancedSizeList[smallestIndex]);
				for(PListType j = 0; j < threadsToDispatch; j++)
				{
...
void Forest::PlantTreeSeedThreadHD(PListType positionInFile, PListType startPatternIndex, PListType numPatternsToSearch, unsigned int threadNum)
{
	...
	printf("Plant Tree Seed earlyApproximation:%lu\n", earlyApproximation);
	...
	printf("Plant Tree Seed positionInFile:%lu\n", positionInFile);
	printf("Plant Tree Seed startPatternIndex:%lu\n", startPatternIndex);
	printf("Plant Tree Seed numPatternsToSearch:%lu\n", numPatternsToSearch);
	printf("Plant Tree Seed threadNum:%d\n", threadNum);
	...
	for (PListType i = startPatternIndex; i < numPatternsToSearch + startPatternIndex; i++)
	{
		...
		printf("Plant Tree Seed temp:%d\n", temp);
		printf("Plant Tree Seed tempIndex:%d\n", tempIndex);
